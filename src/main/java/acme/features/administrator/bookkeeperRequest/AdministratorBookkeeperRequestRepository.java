...
public interface AdministratorBookkeeperRequestRepository extends AbstractRepository {
	...
	BookkeeperRequest findBookkeeperRequestById(int i);
	
	@Query("select b from BookkeeperRequest b where b.accepted = 'false'")
	Collection<BookkeeperRequest> findRequestsNotAcceptedYet();
	
	@Query("select u from UserAccount u where u.id = ?1")
	UserAccount findUserAccountById(int i);
