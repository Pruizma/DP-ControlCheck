...
public interface InvestorApplicationRepository extends AbstractRepository {
	...
	@Query("select a from Application a where a.investor.id= ?1 and a.statement='PENDING' and a.investment.filing!='' and a.investment.filing is not null and not exists(select o from Offer o where o.application.id = a.id)")
